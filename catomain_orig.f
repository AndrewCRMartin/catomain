...
Sure, I'll tack the code onto the end of this message - it isn't very
long. It's in Fortran, but I don't suppose you will be too worried about
that!
...
- David -
	...
	PROGRAM main
c	adds peptides to CA file by (modified) method of Levitt (1977)
	CHARACTER*40 FILE
 	IBRK=3
        IDIS=10
	PRINT*,'Enter Brookhaven filename'
	READ*,FILE
	OPEN(IBRK,FILE=FILE,STATUS='OLD')
	OPEN(IDIS,FILE='main.pdb',STATUS='NEW')
	CALL ATOMG(IBRK)
        CALL CAORD(IDIS)
	STOP
	END

	SUBROUTINE ATOMG(IBRK)
...
C			different chain so exit
	...
	PRINT*,NAT
     ...
     -					 CB(1,NAT-1), 
     -				  SCALE, ARES(NAT-1))
		CALL ADDCB(N(1,NAT-2),CA(1,NAT-2),C(1,NAT-2),CB(1,NAT-2),1.5)
		GOTO 300
...
C	block for CA coordinates and data
	...
	NA=0
	DO 100 I=1,NAT
		WRITE(IBRK,2000) NA+1,'  N   ',ARES(I),I,( N(J,I),J=1,3)
		WRITE(IBRK,2000) NA+2,'  CA  ',ARES(I),I,(CA(J,I),J=1,3)
		WRITE(IBRK,2000) NA+3,'  O   ',ARES(I),I,( O(J,I),J=1,3)
		WRITE(IBRK,2000) NA+4,'  C   ',ARES(I),I,( C(J,I),J=1,3)
		NA=NA+4
		IF(ARES(I).EQ.'GLY') GOTO 100
		NA=NA+1
		WRITE(IBRK,2000) NA,'  CB  ',ARES(I),I,(CB(J,I),J=1,3)
		IF(ARES(I).NE.'PRO') GOTO 100
		CALL ADDCG(CA(1,I),CB(1,I),N(1,I),H(1,I),CG)
		WRITE(IBRK,2000) NA+1,'  CG  ',ARES(I),I,(CG(J),J=1,3)
		WRITE(IBRK,2000) NA+2,'  CD  ',ARES(I),I,(H(J,I),J=1,3)
		NA=NA+2
100 	CONTINUE
	RETURN
2000	FORMAT('ATOM',I7,2A,' A'I4,4X,3F8.3)
	END
